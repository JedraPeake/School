...
var Users = require('../models/clients');
var RolePermissions = require('../models/role-permissions');
    ...
    .post(parseUrlencoded, parseJSON, function (request, response) {
        Passwords.Model.findOne({"userName": request.body.login.userName}, function (error, UserShadow) {
                if (error || !UserShadow) {
                    ...
                    Users.Model.findById(UserShadow.user, function (error, user) {
                            if (user.enabled) {
                                // Now deal with the first message in the authentication protocol
                                ...
                                // User account is diabaled, server will send an error message
                                var accountDisabled = new Logins.Model({
                                    userName: request.body.login.userName,
                                    password: null,
                                    nonce: null,
                                    response: null,
